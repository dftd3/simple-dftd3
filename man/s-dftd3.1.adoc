= s-dftd3(1)
:doctype: manpage

== Name
s-dftd3 - Simple reimplementation of the DFT-D3 dispersion model

== Synopsis
*s-dftd3* [_options_] _input_

== Description

Takes an geometry input to calculate the D3 dispersion correction.
Periodic calculations are performed automatically for periodic input formats
Specify the functional to select the correct parameters.

== Options

*-i, --input* _format_::
Hint for the format of the input file

*--bj* _method_::
Use rational (Becke-Johnson) damping function

*--bj-param* _list_::
Specify parameters for rational damping,
expected order is s6, s8, a1, a2 (requires four arguments)

*--zero* _method_::
Use zero (Chai-Head-Gordon) damping function

*--zero-param* _list_::
Specify parameters for zero damping,
expected order is s6, s8, rs6 (requires three arguments)

*--bjm* _method_::
Use modified rational damping function

*--bjm-param* _list_::
Specify parameters for rational damping,
expected order is s6, s8, a1, a2 (requires four arguments)

*--zerom* _method_::
Use modified zero damping function

*--zerom-param* _list_::
Specify parameters for modified zero damping,
expected order is s6, s8, rs6, beta (requires four arguments)

*--op* _method_::
Use modified zero damping function

*--op-param* _list_::
Specify parameters for optimized power damping,
expected order is s6, s8, a1, a2, beta (requires five arguments)

*--cso* _method_::
Use CSO (C6-scaled only) damping function

*--cso-param* _list_::
Specify parameters for CSO damping,
expected order is s6, a1 (requires two arguments)

*--atm*::
Use ATM three-body dispersion

*--atm-scale* _s9_::
Use scaled ATM three-body dispersion

*--ewald*::
Evaluate the two-body dispersion energy by summation over the reciprocal lattice
instead of a truncated real space sum.
This removes the truncation error of the pair sum, which decays only with the
third power of the cutoff radius.
Requires three-dimensional periodic boundary conditions and a damping function
with a known reciprocal space representation, currently rational (*--bj*, *--bjm*)
and zero (*--zero*, *--zerom* is not supported) damping.
The three-body contribution is still evaluated in real space.

*--ewald-kcut* _kcut_::
Reciprocal space cutoff for the Ewald summation in inverse Bohr.
Implies *--ewald*.
By default the cutoff is derived from the damping radii, a value of 8 is
converged to below one picohartree per atom for rational damping.

*--ewald-mesh* _n_::
Number of points per direction of the particle mesh, rounded up to the next
power of two.
Implies *--ewald*.
By default the mesh is derived from the reciprocal space cutoff.

*--ewald-direct*::
Sum over the reciprocal lattice directly instead of interpolating the structure
factors on a particle mesh.
Implies *--ewald*.
The direct summation is exact up to the reciprocal cutoff but scales
quadratically with the number of atoms, whereas the particle mesh used by
default scales as O(N log N).

*--ghost* _list_::
Disable dispersion contributions from the given 1-based atom indices

*--gcp* _basis_::
Include the geometric counter-poise correction for the given basis set

*--db* _file_::
Load parameters from external data file

*--noedisp*::
Disable writing of dispersion energy to `.EDISP` file

*--json* [_file_]::
Dump results to JSON output (default: `dftd3.json`)

*--grad* [_file_]::
Request gradient evaluation,
write results to _file_ (default: dftd3.txt),
attempts to add to Turbomole gradient and gradlatt files

*--hessian* [_file_]::
Request analytical hessian evaluation,
write results to _file_ (default: `dftd3_hessian.txt`).
The hessian is written as a 3N by 3N matrix in atomic units (Eh/a0^2^),
using the index convention 3*(i-1)+c for the Cartesian component _c_ of atom _i_.
The geometric counter-poise correction is included if *--gcp* is given.

*--property*::
Evaluate dispersion related properties

*--pair-resolved*::
Calculate pairwise representation of dispersion energy

*--citation* [_file_]::
Write citations as bibtex (default: `dftd3.bib`)

*-v, --verbose*::
Show more, can be used multiple times

*-s, --silent*::
Show less, use twice to supress all output

*--version*::
Print program version and exit

*--help*::
Show this help message
