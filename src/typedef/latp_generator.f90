...
module d3def_latp_generator
   use iso_fortran_env, only: wp => real64
   implicit none
   private

   public :: d3_latp_generator


   !> Lattice point iterator
   type :: d3_latp_generator
      private
...
subroutine latpgen_new(self, lattice, inv_lat, cutoff, neg_extension, &
      ...
      &                exclude_origin)
   !> Instance
   class(d3_latp_generator), intent(out) :: self
   !> Lattice vectors
...
pure subroutine latpgen_get_next_point(self, latticePoint, finished)
   !> Instance
   class(d3_latp_generator), intent(inout) :: self
   !> Next lattice point
...
pure subroutine latpgen_get_all_points(self, latticePoints)
   !> Instance
   class(d3_latp_generator), intent(inout) :: self
   !> Lattice points
...
pure subroutine get_next_point(ranges, point)
...
pure subroutine get_ranges(dist, recVec2p, pos_ext, neg_ext, ranges)
...
end module d3def_latp_generator
