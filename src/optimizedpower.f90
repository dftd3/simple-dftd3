...
pure subroutine get_damping_kernel(self, izp, jzp, rvdw, r4r2, r2, c6, &
   ...
   real(wp) :: rrij, r0ij, r0ij2, r0ij6, r0ij8, u, ab, abr0ij6, abr0ij8, q
   real(wp) :: d6, d8, d6d, d8d, d6dd, d8dd
   real(wp) :: t6, t8, t6d, t8d, t6dd, t8dd, p, pd, pdd
   ...
   d6 = u**3 + abr0ij6*u**(-q)
   d8 = u**4 + abr0ij8*u**(-q)
   d6d = 3.0_wp*u*u - q*abr0ij6*u**(-q - 1.0_wp)
   d8d = 4.0_wp*u**3 - q*abr0ij8*u**(-q - 1.0_wp)
   d6dd = 6.0_wp*u + q*(q + 1.0_wp)*abr0ij6*u**(-q - 2.0_wp)
   d8dd = 12.0_wp*u*u + q*(q + 1.0_wp)*abr0ij8*u**(-q - 2.0_wp)

   t6 = 1.0_wp/d6
   t8 = 1.0_wp/d8
   t6d = -d6d*t6*t6
   t8d = -d8d*t8*t8
   t6dd = -d6dd*t6*t6 + 2.0_wp*d6d*d6d*t6**3
   t8dd = -d8dd*t8*t8 + 2.0_wp*d8d*d8d*t8**3

   p = self%s6*t6 + self%s8*rrij*t8
   pd = self%s6*t6d + self%s8*rrij*t8d
   pdd = self%s6*t6dd + self%s8*rrij*t8dd
...
subroutine get_pairwise_dispersion2(self, mol, trans, cutoff, width, rvdw, r4r2, c6, energy)
   ...
   !$omp private(iat, jat, izp, jzp, jtr, vec, r2, r1, r0ij, rrij, c6ij, t6, &
   !$omp& t8, edisp, dE, rb, ab, sw, dswdr) &
   !$omp shared(energy) &
   !$omp private(energy_local)
